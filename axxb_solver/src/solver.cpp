...
Eigen::Matrix3d solve_r(const std::vector<Eigen::Matrix4d> &T_a,
  ...
  Eigen::Vector3d k_r_prime =
      S.bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(v);

  double theta = 2 * std::atan(k_r_prime.norm());

  Eigen::Vector3d k_r_axis = k_r_prime / k_r_prime.norm();
  // std::cout << (Eigen::Matrix3d::Identity() * std::cos(theta) +
  // std::sin(theta) * skew(k_r_axis) + (1 - std::cos(theta)) * k_r_axis *
  // k_r_axis.transpose()).transpose() << std::endl;
